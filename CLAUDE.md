...
- **`cli/`** — the CLI. `cli/mod.rs` is clap parsing + `run()`/dispatch plus the cross-cutting helpers handlers share (`state_of`/`replay`, `parse_field_ops`, `confirm`); each subcommand handler is its own file under `cli/commands/`. Handlers take `&impl EventStore`, not `FileStore`. Beyond the README's commands there is `show` (one task; human output is a vertical `field: value` record via `format::render_record`, untruncated — `--format json`/`jsonl` go the normal route), `status`, `config get/set/list`, `undo` (reverse the last N events — see invariants), the `dep` group (`add`/`remove`/`list`/`tree`/`cycles` — typed relationship edges validated against `[relationships]`, via `cmd_dep_group`/`DepAction`; `dep` replaced the old top-level `block`/`unblock`), and `resolve` (which also prunes orphaned events). The display flag for "every field" is `--full` (not `--all`).
