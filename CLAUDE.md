...
The same boundary pattern gives the **canonical status key**: events/baseline always store `status` (`model::STATUS_KEY`); `[workflow] status_field` is a *display* name — `state_of` renames canonical→display on read, `cli::canonicalize_fields` maps display→canonical on write (rejecting the canonical spelling while a different display name is configured). Renaming the field in config is therefore free — no data migration; the `canonical-status-key` repair pass covers stores renamed before this existed — and clones with different display configs merge cleanly. Mind the two state views: raw/engine state (vet, undo, merge, compaction) is always canonical, `state_of` state is display-named — pass the matching field name to `is_done`/graph helpers.
