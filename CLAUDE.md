...
- **`cli/`** — the CLI. `cli/mod.rs` is clap parsing + `run()`/dispatch plus the cross-cutting helpers handlers share (`state_of`/`replay`, `parse_fields`, `confirm`); each subcommand handler is its own file under `cli/commands/`. Handlers take `&impl EventStore`, not `FileStore`. Beyond the README's commands there is `show` (one task, all fields), `status`, `config get/set/list`, `undo` (reverse the last N events — see invariants), and `resolve` (which also prunes orphaned events). The display flag for "every field" is `--full` (not `--all`).
