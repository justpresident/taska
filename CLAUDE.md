...
- **`graph.rs`** — dependency DAG over `petgraph`: cycle detection, topological sort, and `ready_tasks` (not-done tasks whose deps are all done). The graph is built from **blocker edges** (`graph::blocker_edges`): any relationship whose type is `blocker`/`hierarchy` (per `RelationshipConfig::blocker_types`, which always includes the default `depends_on`). All edges, `depends_on` included, live in `TaskState::relationships`; the `deps` column is that whole typed map (`{type: [targets…]}` in json, labeled type groups in human output — gating types bold, info dim). Informational (`info`) relationships never gate readiness/cycles/`dep tree`. Callers pass the blocker-name set in, so `graph.rs` stays free of `config`.
