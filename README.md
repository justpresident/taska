...
| `ta completions <shell>` | Print a static shell-completion script (`bash`, `zsh`, `fish`, `powershell`, `elvish`) generated from the live CLI, so it never drifts. Redirect it onto your shell's completion path, e.g. `ta completions bash > ~/.local/share/bash-completion/completions/ta` |
