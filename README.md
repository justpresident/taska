...
| `ta dep tree [<task> ...]` | ASCII dependency tree with a shortened title per node, colored on a TTY (done tasks dimmed + `[x]`). Shows the exact graph by default - never spliced; `--open` prunes fully-resolved branches. `--sort`/`--reverse` order siblings. Subtasks are tagged `[subtask]`, a parent rolls up `[subtasks done/total]`, shared nodes collapse, cycles are flagged |
