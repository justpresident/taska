...
| `ta completions <shell>` | Print the completion setup for a shell (`bash`, `zsh`, `fish`, `powershell`, `elvish`). **Source** it into your rc - `echo 'source <(ta completions bash)' >> ~/.bashrc` - and TAB then completes subcommands, flags, and live **task ids, filter fields, and column names** read from the store. Generated from the live CLI, so it never drifts |
