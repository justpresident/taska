...
/// `migrate`: run the stacking on-disk format migrations and rewrite both files.
/// Returns the per-migration `(id, event_count)` report; empty = already current.
pub fn migrate(store: &impl EventStore) -> Result<Vec<(&'static str, usize)>, DynError> {
...
/// One `rename` outcome.
///
/// The (display) destination, the source, how many records moved, and how many
/// were KEPT because converting them would have violated the schema (type
/// destinations only).
...
pub fn schema(
...
) -> Result<SchemaRepairReport, DynError> {
    let config = store.config();
    let mut log = store.load_mutations()?;
    let mut baseline = store.load_baseline()?;

    let rename_outcome = match rename {
        Some(spec) => Some(apply_rename(&mut log, &mut baseline, spec, config)?),
        None => None,
    };
    let renamed = rename_outcome.as_ref().map_or(0, |o| o.moved);
    ...
    let changed = renamed > 0 || !typed.is_empty() || !fixes.is_empty();
    if changed {
