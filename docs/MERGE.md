...
is ~1/4 `Create`s; the rest are `Update`s and random `AddEdge`s, with the dep share
...
The replay speedup widens as history deepens - `~2`x at 100k, `~3.3`x at 500k - at a
roughly constant `~2.2`x smaller footprint. And it understates the real win: this
